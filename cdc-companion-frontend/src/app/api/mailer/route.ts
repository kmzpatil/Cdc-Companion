...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!authHeader || authHeader !== `Bearer ${process.env.MAILER_SECRET}`) {
    ...
    if (action === 'REVIEW') {
      const { to, userName, reviewComments, aiSuggestions } = payload;
      await sendReviewEmail({ to, userName, reviewComments, aiSuggestions });
